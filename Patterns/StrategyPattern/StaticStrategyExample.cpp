...
namespace StaticStrategyPatternExample {
    ...
    private:
        // std::unique_ptr<IListStrategy> m_list_strategy;
        TList                             m_list;  // Ein Objekt des Typs TList
        std::ostringstream m_oss;

    public:
        void append_list(std::initializer_list<std::string> items)
        {
            // m_list_strategy->start(m_oss);    -> ist INDIREKT // virtual
            m_list.start(m_oss);                  // . ist DiREKT !!!
            for (const auto& item : items) {
    ...
    struct MarkdownListStrategyEx  // KEINE BASISKLASSE !!!
    {
...
void test_static_strategy_example ()
{
    ...
    // Markdown
    TextProcessorEx<MarkdownListStrategyEx> tp1{}; // STATISCH // Übersetzungszeit
