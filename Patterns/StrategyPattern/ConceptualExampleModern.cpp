...
namespace StrategyConceptualExampleModern {
    ...
    std::string(*fp) (const std::vector<std::string>&);  // Definition eines Funktionszeigers , C Stil

    //                               In <>:  Signatur der Funktion
    ...
    // free lambda function  doAlgorithmA_Lambda

    auto doAlgorithmA_Lambda = [](const std::vector<std::string>& data) {
        std::string result{};
    ...
    class Context
    {
         ...
         */
    private:
        std::function < std::string(const std::vector<std::string>&)>  m_strategy;
      //  Strategy m_strategy;
    ...
    public:
        ...
        void doSomeBusinessLogic() const
        {
            ...
            std::string result{ m_strategy(someStrings) };  //  std::function: m_strategy dieses aufrufen
